...
const UploadPage: React.FC = () => {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const parsedData = await parseFileWithGemini(
        base64,
        file.type,
        file.name
      );
      ...
      setError(
        "Analysis failed. Please check your network or try a different file."
      );
      setIsLoading(false);
    }
  };
  ...
  const howItWorksSteps = [
    {
      ...
      title: "Secure Upload",
      description:
        "Upload your project scope in PDF or Excel format. Our system supports high-density technical documents.",
    },
    {
      icon: <Search className="w-6 h-6" />,
      title: "AI Semantic Parsing",
      description:
        "ScopeLens uses Gemini's advanced reasoning to identify milestones, deliverables, and estimated efforts.",
    },
    {
      icon: <LayoutDashboard className="w-6 h-6" />,
      title: "Interactive Analysis",
      description:
        "Explore your project through a structured dashboard with filtered views and detailed breakdowns.",
    },
    {
      icon: <FileDown className="w-6 h-6" />,
      title: "Smart Export",
      description:
        "Generate professionally formatted PDF reports for clients or internal stakeholders in one click.",
    },
  ];
          ...
          <div className="bg-white rounded-[3rem] shadow-2xl max-w-4xl w-full p-8 md:p-12 transform transition-all animate-in zoom-in-95 duration-300 relative overflow-hidden max-h-[90vh] overflow-y-auto">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
              {howItWorksSteps.map((step, i) => (
                <div
                  key={i}
                  className="flex flex-col sm:flex-row space-y-4 sm:space-y-0 sm:space-x-6 p-6 rounded-[2rem] bg-slate-50/50 border border-slate-100 group hover:bg-white hover:shadow-xl transition-all duration-500"
                >
            ...
            <div className="mt-12 flex justify-center">
              <Button
                variant="primary"
                onClick={() => setShowHowItWorks(false)}
                className="px-10 py-4 rounded-2xl shadow-xl w-full sm:w-auto"
              >
                Got it, let's start
              </Button>
            </div>
              ...
              className={`
                ...
                ${
                  file
                    ...
                    : "border-slate-200/80 bg-white/50 hover:border-indigo-400 hover:bg-white/90 hover:shadow-2xl hover:-translate-y-1"
                }
                ...
                <div className="flex flex-col items-center animate-in zoom-in-95 duration-500">
                  <div className="w-20 h-20 md:w-24 md:h-24 bg-indigo-600 text-white rounded-[1.5rem] md:rounded-[2rem] flex items-center justify-center mb-6 shadow-2xl shadow-indigo-200 animate-in slide-in-from-bottom-4">
                    {file.type.includes("pdf") ? (
                      ...
                      <FileIcon className="w-10 h-10 md:w-12 md:h-12" />
                    )}
                  </div>
                  <div className="flex items-center space-x-3 bg-white px-6 md:px-8 py-3 md:py-4 rounded-2xl shadow-xl border border-slate-50">
                    <span className="text-base md:text-lg font-black text-slate-900 truncate max-w-[180px] md:max-w-[280px]">
                      {file.name}
                    </span>
                    {!isLoading && (
                      <button
